...
def login():
     ...
     # it maintains the loop of our program until the program has come to an end
     ...
     if len(username1)==0 and len(password1)==0:
          ...
          if category == 'Admin':
             for i in admin_details:
               ...
               # to list the files present in the present path
                              ...
                              for i in d:
                                   if d['Username'] == username1:
                                        if d['Password'] == password1 :
                                             info = 'Welcome - ' + d['Name']
                                             tmsg.showinfo('Welcome',info,parent=login_screen)
                                             # creating a temp file to link the two py files
...
def discount(l,d):
    disc_days = []
    today = date.today()
    my_date = today.strftime("%d/%m/%Y") # today's date
    y = int(my_date.split('/')[-1]) 
    for a in holidays.India(years = y).items():
        disc_days += [[a[0],a[1]]]# date and festival name
                     
    for k in disc_days:
        k[0] = k[0].strftime("%d/%m/%Y")# festival date format change to dd/mm/yyyy
        if my_date == k[0]:
            for i in l:
...
def register_user():
     ...
     if len(username_info)==0 and len(password_info)==0 or len(username_info)==0 and len(name_info)==0 and len(password_confirm_info)==0 and str(phone_nos_info)=='0':
          ...
          if len(password_info or password_confirm_info) >= 8 :
               ...
               # to check whether the user already registered if both username and password matches
                    ...
                    # same name but username and password different 
                    else:
...
def discount_details():
    ...
    menu_tabel1.column("total",width=75,anchor='center') 
    menu_tabel1.column("name",width=25,anchor='center') 

    #heading
    menu_tabel1.heading("name",text="Name")
    menu_tabel1.heading("total",text="Total Price(discount not included)")
